...
void TaskGenericFuntion(void * parm)
{
	int val = (int)parm;
	OLED_ShowNum(0, 16, val, 2, 16,1);
	OLED_Refresh();
	vTaskDelete(NULL);
}
...
void Task2Function(void * param)
{
	...
	while (1)
	{
		...
		OLED_Refresh();
		printf("asd");
		vTaskDelayUntil(&t, 500);		//启动时刻的间隔一致
		if(i == 10)
			vTaskSuspend(xHandleTCB3);		//阻塞任务
		if(i == 15)
		{
			xTaskCreate(TaskGenericFuntion, "ParmTestFuntion", 100, (void *)123, 1, &xHandleTCBGeneric);
		}
		if(i == 20)
		{
			vTaskResume(xHandleTCB3);		//释放任务
		}
...
int main( void )
{
  ...
  debug();
	...
	xTaskCreate(Task1Function, "Task1", 100, NULL, 1, &xHandleTask1);
	xTaskCreate(Task2Function, "Task2", 1000, NULL, 2, &xHandleTask2);
